...
const defaults = { name: "Bia", pin: "1234", streak: 0, history: [], reward: false, activeSession: null };
let state = loadState();
let session = null;
...
function renderCard() {
  ...
  $("#definition").textContent = word.definition; $("#synonyms").textContent = word.synonyms;
...
function showReward() { createConfetti(); showScreen("rewardScreen"); }
...
$("#settingsButton").addEventListener("click", () => { $("#studentName").value = state.name; $("#masterPin").value = ""; $("#settingsDialog").showModal(); });
$("#settingsForm").addEventListener("submit", (event) => { if (event.submitter?.value !== "save") return; event.preventDefault(); const pin = $("#masterPin").value; if (pin && !/^\d{4}$/.test(pin)) return toast("O PIN precisa ter 4 números."); state.name = $("#studentName").value.trim() || "Bia"; if (pin) state.pin = pin; saveState(); $("#settingsDialog").close(); refreshDashboard(); toast("Configurações salvas!"); });
$("#resetData").addEventListener("click", () => { if (!confirm("Apagar todo o progresso e iniciar novamente?")) return; state = { ...defaults }; saveState(); $("#settingsDialog").close(); refreshDashboard(); toast("Progresso reiniciado."); });
$("#validateButton").addEventListener("click", () => { $("#pinAttempt").value = ""; $("#pinError").textContent = ""; $("#pinDialog").showModal(); });
$("#pinForm").addEventListener("submit", (event) => { if (event.submitter?.value !== "validate") return; event.preventDefault(); if ($("#pinAttempt").value !== state.pin) { $("#pinError").textContent = "PIN incorreto. Peça ajuda ao Master."; return; } state.streak = 0; state.reward = false; state.history = []; saveState(); $("#pinDialog").close(); refreshDashboard(); showScreen("dashboardScreen"); toast("Recompensa validada. Um novo ciclo começou!"); });

if ("serviceWorker" in navigator && location.protocol.startsWith("http")) navigator.serviceWorker.register("service-worker.js");
refreshDashboard(); if (state.reward) showReward();
